...
export default function IbanCalculator() {
  ...
  const [blzStatus, setBlzStatus] = useState<"loading" | "ready" | "error">("loading");
  const [blzMap, setBlzMap] = useState<Map<string, BlzEntry> | null>(null);

  useEffect(() => {
    loadBlzData()
      .then(({ map }) => { setBlzMap(map); setBlzStatus("ready"); })
      .catch(() => setBlzStatus("error"));
  }, []);
  ...
  const lookupBlz = useCallback((val: string) => {
    ...
    setBankInfo(blzMap.get(val) ?? null);
  }, [country, blzMap]);

  // Auto-calculate for structured countries when both fields are complete
  useEffect(() => {
    if (!structured) return;
    const blzClean = blz.replace(/\s/g, "");
    const kontoClean = konto.replace(/\s/g, "");
    if (blzClean.length > 0 && kontoClean.length > 0 &&
        blzClean.length <= structured.blzLen && kontoClean.length <= structured.kontoLen) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (structured) return; // auto-calculated
    setError(null);
    setIban(null);
    setCopied(false);
        ...
        <>
          ...
          <div>
            ...
            {country === "DE" && blz.length === structured.blzLen && (
              <div className={`mt-2 rounded-lg px-3 py-2 text-sm ${bankInfo ? "border border-zinc-100 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800/50" : ""}`}>
                {blzStatus === "ready"
                  ? bankInfo
                  ...
                  : blzStatus === "error"
                  ? <span className="text-red-500">BLZ-Daten konnten nicht geladen werden</span>
          ...
          <div>
            ...
            <input
              ...
              inputMode="numeric"
              value={konto}
              onChange={(e) => { setKonto(e.target.value.replace(/\D/g, "")); setError(null); }}
